...
const Series = () => {
  ...
  const CollectionRef = collection(db, 'discussionsSeries')
  ...
  const getSubcategory = async () => {
    ...
    q = query(q, where('category', '==', 'Discussions'))
    const data = await getDocs(q).then(function (data) {
      setSubcategoryData(
        data.docs.map((doc) => ({
          ...doc.data(),
  ...
  const uploadMedia = (e) => {
    ...
    if (!file) return
    const storageRef = ref(storage, `discussionsSeries_thumbnail/${file.name}`)
  ...
  const deleteMedia = async (newUrl) => {
    ...
    const desertRef = ref(storage, `discussionsSeries_thumbnail/${httpsReference.name}`) //url to file name get for delete from storage
    // Delete the file
  ...
  const update = async (url) => {
    ...
    setLoading(true)
    await updateDoc(doc(db, 'discussionsSeries', editId), {
  ...
  const edit = async (id) => {
    setEditVisible(true)
    const docRef = doc(db, 'discussionsSeries', id)
  ...
  const Delete = async () => {
    ...
    deleteDoc(doc(db, 'discussionsSeries', editId)).then(() => {
      setEditVisible(false)
      getSeries()
    })
  }
  ...
  return (
    <>
      ...
      <CModal alignment="center" visible={visible} onClose={() => handleClose()}>
        <CModalHeader>
          <CModalTitle>Create Discussions Series</CModalTitle>
        </CModalHeader>
        <CModalBody>
          <CRow className="mb-3">
      ...
      <CModal alignment="center" visible={editVisible} onClose={() => handleClose()}>
        <CModalHeader>
          <CModalTitle>UPDATE DISCUSSIONS SERIES</CModalTitle>
        </CModalHeader>
        <CModalBody>
          <CRow className="mb-3">
            <CFormLabel htmlFor="staticEmail" className="col-sm-4 col-form-label">
