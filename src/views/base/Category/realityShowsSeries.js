...
const Series = () => {
  ...
  const CollectionRef = collection(db, 'realityShowsSeries')
  ...
  const getSubcategory = async () => {
    ...
    q = query(q, where('category', '==', 'Reality Shows'))
    const data = await getDocs(q).then(function (data) {
      setSubcategoryData(
        data.docs.map((doc) => ({
          ...doc.data(),
  ...
  const uploadMedia = (e) => {
    ...
    if (!file) return
    const storageRef = ref(storage, `realityShowsSeries_thumbnail/${file.name}`)
  ...
  const deleteMedia = async (newUrl) => {
    ...
    const desertRef = ref(storage, `realityShowsSeries_thumbnail/${httpsReference.name}`) //url to file name get for delete from storage
    // Delete the file
  ...
  const update = async (url) => {
    ...
    setLoading(true)
    await updateDoc(doc(db, 'realityShowsSeries', editId), {
  ...
  const edit = async (id) => {
    setEditVisible(true)
    const docRef = doc(db, 'realityShowsSeries', id)
  ...
  const Delete = async () => {
    ...
    deleteDoc(doc(db, 'realityShowsSeries', editId)).then(() => {
      setEditVisible(false)
      getSeries()
    })
  }
  ...
  return (
    <>
      ...
      <CModal alignment="center" visible={visible} onClose={() => handleClose()}>
        <CModalHeader>
          <CModalTitle>Create Reality Shows Series</CModalTitle>
        </CModalHeader>
        <CModalBody>
          <CRow className="mb-3">
      ...
      <CModal alignment="center" visible={editVisible} onClose={() => handleClose()}>
        <CModalHeader>
          <CModalTitle>UPDATE REALITY SHOWS SERIES</CModalTitle>
        </CModalHeader>
        <CModalBody>
          <CRow className="mb-3">
            <CFormLabel htmlFor="staticEmail" className="col-sm-4 col-form-label">
    ...
    <CRow>
      <CCol xs={12}>
        <CCard className="mb-4">
          <CCardHeader>
            <h2>REALITY SHOWS SERIES LIST</h2>
          </CCardHeader>
