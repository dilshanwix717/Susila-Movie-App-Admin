...
const webSeries = () => {
    ...
    const CollectionRef = collection(db, 'webSeries')
    ...
    const getSeries = async () => {
        ...
        console.log('------Web series List---------------', myData);
    }
    ...
    const uploadMedia = (e) => {
        ...
        if (!file) return
        const storageRef = ref(storage, `webSeries_thumbnail/${file.name}`)
    ...
    const deleteMedia = async (newUrl) => {
        ...
        const desertRef = ref(storage, `webSeries_thumbnail/${httpsReference.name}`) //url to file name get for delete from storage
        // Delete the file
    ...
    const update = async (url) => {
        ...
        setLoading(true)
        await updateDoc(doc(db, 'webSeries', editId), {
    ...
    const edit = async (id) => {
        setEditVisible(true)
        const docRef = doc(db, 'webSeries', id)
    ...
    const Delete = async () => {
        ...
        deleteDoc(doc(db, 'webSeries', editId)).then(() => {
            setEditVisible(false)
            getSeries()
        })
    }
    ...
    return (
        <>
            ...
            <CModal alignment="center" visible={visible} onClose={() => handleClose()}>
                <CModalHeader>
                    <CModalTitle>Create Web Series</CModalTitle>
                </CModalHeader>
                <CModalBody>
                    <CRow className="mb-3">
            ...
            <CModal alignment="center" visible={editVisible} onClose={() => handleClose()}>
                <CModalHeader>
                    <CModalTitle>UPDATE WEB SERIES</CModalTitle>
                </CModalHeader>
                <CModalBody>
                    <CRow className="mb-3">
                        <CFormLabel htmlFor="staticEmail" className="col-sm-4 col-form-label">
                    ...
                    <form onSubmit={uploadMedia}>
                        ...
                        <div className="row justify-content-md-center">
                            ...
                            <CCol>
                                <CButton color="danger" onClick={() => setDeleteVisible(true)}>
                                    DELETE
                                </CButton>
                            </CCol>
                        </div>
            ...
            <CModal alignment="center" visible={deleteVisible} onClose={() => setDeleteVisible(false)}>
                ...
                <CModalFooter>
                    ...
                    <CButton color="primary" onClick={() => Delete(editId)}>
                        Yes, Delete
                    </CButton>
                </CModalFooter>
            </CModal>
        ...
        <CRow>
            <CCol xs={12}>
                <CCard className="mb-4">
                    <CCardHeader>
                        <h2> WEB SERIES LIST</h2>
                    </CCardHeader>
                    <CCardBody>{webSeries()}</CCardBody>
                </CCard>
            </CCol>
        </CRow>
    )
