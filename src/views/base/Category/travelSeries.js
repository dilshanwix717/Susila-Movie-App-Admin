...
const Series = () => {
  ...
  const CollectionRef = collection(db, 'travelSeries')
  ...
  const uploadMedia = (e) => {
    ...
    if (!file) return
    const storageRef = ref(storage, `travelSeries_thumbnail/${file.name}`)
  ...
  const deleteMedia = async (newUrl) => {
    ...
    const desertRef = ref(storage, `travelSeries_thumbnail/${httpsReference.name}`) //url to file name get for delete from storage
    // Delete the file
  ...
  const update = async (url) => {
    ...
    setLoading(true)
    await updateDoc(doc(db, 'travelSeries', editId), {
  ...
  const edit = async (id) => {
    setEditVisible(true)
    const docRef = doc(db, 'travelSeries', id)
  ...
  const Delete = async () => {
    ...
    deleteDoc(doc(db, 'travelSeries', editId)).then(() => {
      setEditVisible(false)
      getSeries()
    })
  }
  ...
  return (
    <>
      ...
      <CModal alignment="center" visible={visible} onClose={() => handleClose()}>
        <CModalHeader>
          <CModalTitle>Create Travel Series</CModalTitle>
        </CModalHeader>
        <CModalBody>
          <CRow className="mb-3">
      ...
      <CModal alignment="center" visible={editVisible} onClose={() => handleClose()}>
        <CModalHeader>
          <CModalTitle>UPDATE TRAVEL SERIES</CModalTitle>
        </CModalHeader>
        <CModalBody>
          <CRow className="mb-3">
            <CFormLabel htmlFor="staticEmail" className="col-sm-4 col-form-label">
